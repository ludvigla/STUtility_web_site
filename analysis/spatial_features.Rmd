...
ST.FeaturePlot(object = se, features = "clusters_ICA", dark.theme = T, cols = col_vector, pt.size = 0.5)
...
DimPlot(se, reduction = "umap", cols = col_vector) + DarkTheme()
...
<br>
```{r plot_clusters_split, fig.width=8, out.width='100%'}

ST.FeaturePlot(object = se, features = "clusters_ICA", dark.theme = T, cols = col_vector, pt.size = 0.1, split.labels = T, indices = 1)

```
<br>
We can take a look a specific look at some of the most variable features.
<br>
...
As the white knights of science, we promote the use of orthonogal data to validate results :-)
...
Lets try this out by an example. Looking at \ref{plot_clusters_split}, lets say we are interested in cluster 19, and want to answer - "Which genes are significantly upregulated in this region compared to the overall expression across the tisse?"
...
Note that the clusters were already set as the Seurat objects levels. Type `levels(se)` to see the current levels of your object. If other clusters, annotations etc are of interest, set this before by specifing `Idents(se) <- `
...
<br>
```{r allen_example, fig.height=12, fig.width=6, out.width='100%'}
# CHANGE
