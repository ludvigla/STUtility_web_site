...
STUtility also includes an additional method for finding genes with spatial patterns across the tissue. The ranking method makes use neighbourhood networks to compute the spatial lag for each gene, here defined as the summed expression of that gene in neighbouring spots. Each gene is then ranked by the correlation between the lag vector and the original expression vector. The output is a ranked list of the genes with highest correlation between the two vectors. 
